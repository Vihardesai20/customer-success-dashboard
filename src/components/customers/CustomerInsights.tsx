  ...
  return (
    <section aria-labelledby="account-insights-heading" className="mt-8">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        >
          {customer.keyRisks.length === 0 ? (
            <EmptyInsight message="No key risks captured for this account yet." />
          ) : (
        ...
        >
          {customer.recentActivities.length === 0 ? (
            <EmptyInsight message="No recent activity recorded in the current portfolio data." />
          ) : (
            <ul className="space-y-3">
              {customer.recentActivities.map((activity) => (
                <li
                  key={`${activity.date}-${activity.summary}`}
        ...
        >
          {customer.successGoals.length === 0 ? (
            <EmptyInsight message="No success goals are available for this account." />
          ) : (
        ...
        >
          {customer.nextActions.length === 0 ? (
            <EmptyInsight message="No recommended actions are available for this account." />
          ) : (
            <ul className="space-y-2">
